      ...
      <div className='hide-scroll overflow-x-auto md:container md:overflow-visible'>
        <ul className='flex flex-nowrap gap-3 md:grid md:grid-cols-2 md:gap-5 lg:grid-cols-3'>
          {data.map((destination, index) => (
            <li
              key={index}
              className='mb-3 min-h-[222px] min-w-[284px] rounded-3xl first:pl-4 last:pr-4 md:mb-0 md:w-full md:first:pl-0 md:last:pr-0 lg:w-full'
              style={{
                boxShadow: '0 4px 10px rgba(0, 0, 0, 0.15)',
              }}
            >
              <article className='relative h-full flex-shrink-0 overflow-hidden rounded-3xl bg-white shadow-lg transition-shadow duration-300 hover:shadow-lg'>
                <img
