...
export function Typography({ className, children, variant }: ITypography) {
  switch (variant) {
    ...
    case 'h3':
      return <h3 className={`${className} text-[40px] font-medium`}>{children}</h3>;
    case 'h4':
      return <h4 className={`${className} text-[32px] font-medium`}>{children}</h4>;
    case 'h5':
      return <h5 className={`${className} text-2xl font-medium`}>{children}</h5>;
    case 'subtitle1':
    ...
    case 'l-bold':
      return <span className={`${className} text-xl font-medium`}>{children}</span>;
    case 'm-bold':
      return (
        <span className={`${className} text-base font-medium`}>{children}</span>
      );
    case 's-bold':
