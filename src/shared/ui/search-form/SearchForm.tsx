...
import { selectTabBarSearchForm } from '@/app/rtk/slices/tabBarSearcForm';
...
export function SearchForm({ className }: SearchFormProps) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      const response = await fetch(`https://${process.env.NEXT_PUBLIC_API}`, {
        method: 'POST',
        headers: {
  ...
  return isClient ? (
    <form
      ...
      className={`${formStyle} ${formShadow} ${className} search-form mx-auto ${tabClick === 'Отели' ? 'w-fit' : ''}`}
    >
      <label
        htmlFor='DepartureCity'
        className={`${LabelStyle}  ${tabClick === 'Отели' ? 'hidden' : 'block xl:w-[14%]'} `}
      ...
      >
        ...
        />
        <input
          className={`${inputStyle} ${reqData ? '' : 'mt-[-12px] font-[500]'} ${screen.width < 1280 && 'text-sm'}`}
          style={{ gridArea: 'input1' }}
          id='DepartureCity'
          type='text'
        ...
        {errors.DepartureCity && (
          <div className='text-red-primary-800'>{errors.DepartureCity.message}</div>
        )}
      </label>

      <label
       htmlFor='Where'
        className={`${LabelStyle} ${tabClick === 'Отели' ? '' : 'xl:w-[14%]'} `}
      ...
      >
        ...
        />
        <input
          className={`${inputStyle} ${reqDataSecond.length > 1 ? '' : 'mt-[-12px] font-[500]'} ${screen.width < 1280 && 'text-sm'}`}
          id='Where'
          type='text'
          placeholder='Куда'
          {...register('Where', {
        ...
        />
        {errors.Where && (
          <div className='text-red-primary-800'>{errors.Where.message}</div>
        )}
        <PopupWindow
          ref={whereRef}
          children={
      ...
      >
        ...
        />
        <input
          className={`${inputStyle} ${selectedDateArrival ? '' : 'mt-[-12px] font-[500]'} ${screen.width < 1280 && 'text-sm'}`}
          style={{ gridArea: 'input3' }}
          id='ArrivalDate'
          type='text'
        ...
        />
        {errors.ArrivalDate && (
          <div className='text-red-primary-800'>{errors.ArrivalDate.message}</div>
        )}
        <SvgSprite
          name='calendar'
          width={20}
      ...
      >
        ...
        />
        <input
          className={`${inputStyle} ${selectedDateDeparture ? '' : 'mt-[-12px] font-[500]'} ${screen.width < 1280 && 'text-sm'}`}
          id='DepartureDate'
          type='text'
        ...
        {errors.DepartureDate && (
          <div className='text-red-primary-800'>{errors.DepartureDate.message}</div>
        )}
        <SvgSprite
          name='calendar'
          width={20}
      ...
      >
        ...
        />
        <div
          className={`absolute ${guests !== 'Количество гостей' ? 'left-[2px] top-[12px] font-[500]' : 'left-[-7px] top-[-4px]'} text-black ${screen.width < 1280 && 'text-sm'}`}
        >
          <Select
            id='GuestsSelect'
            arrowHidden={true}
            ...
            getValue={(value) => {
              setValue('GuestsSelect', value); // Передаю option в Form
              setGuests(value);
            }}
          />
        </div>
      ...
      <ButtonCustom
        ...
        size='m'
        type='submit'
        className={`mt-2 ${tabClick === 'Отели' && screen.width > 1280 ? 'w-max' : 'w-full'} xl:mt-0`}
